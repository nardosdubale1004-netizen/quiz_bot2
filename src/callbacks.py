...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, engine):
    ...
    try:
        if action == "ans":
            ...
            await query.answer("Generating Answer Summary Sheet...")

            user_id = query.from_user.id
            is_correct = (user_selection == question_data['correct_option'])

            perf_card = await asyncio.to_thread(
                process_user_score, user_id, mid_key, question_data['id'], is_correct, user_selection,
                private_message_id=None, show_derivation=True, show_perf=False
            )
            ...
            if active_is_photo:
                    ...
                    async with httpx.AsyncClient() as client:
                        ...
                        if resp and resp.status_code == 200:
                            ...
                            follow_up = await send_rich_message_safe(
                                context.bot, chat_id=query.message.chat_id, html_content=full_text, reply_to_message_id=query.message.message_id
                            )
                ...
                await edit_rich_message_safe(
                    context.bot, chat_id=query.message.chat_id, message_id=query.message.message_id, html_content=explanation_html, reply_markup=reveal_kb
                )
            return
            ...
            perf_card = await asyncio.to_thread(
                process_user_score, user_id, mid_key, question_data['id'],
                (user_selection == question_data['correct_option']), user_selection,
                show_derivation=show_derivation, show_perf=show_perf
            )

            if perf_card and "selected_option" in perf_card:
                user_selection = perf_card["selected_option"]

            explanation_html = UIFactory.build_answered_view(question_data, d_id, user_selection, show_derivation=show_derivation, show_perf=show_perf, perf_card=perf_card)
            ...
            await edit_rich_message_safe(
                context.bot, chat_id=query.message.chat_id, message_id=query.message.message_id, html_content=explanation_html, reply_markup=kb
            )
            return
            ...
            if not show_derivation and not show_perf:
                if existing_response and existing_response.get("private_followup_message_id"):
                    try:
                        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=existing_response["private_followup_message_id"])
                    except Exception:
                        pass
                    await asyncio.to_thread(db_update_private_message_ids, user_id, mid_key, private_followup_message_id=None)
            else:
                full_text = UIFactory.build_answered_view(question_data, d_id, user_selection, show_derivation=show_derivation, show_perf=show_perf, perf_card=perf_card, continuation=True)
                ...
                if existing_response and existing_response.get("private_followup_message_id"):
                    try:
                        await edit_rich_message_safe(
                            context.bot, chat_id=query.message.chat_id, message_id=existing_response["private_followup_message_id"], html_content=full_text, reply_markup=None
                        )
                    except Exception:
                        follow_up = await send_rich_message_safe(
                            context.bot, chat_id=query.message.chat_id, html_content=full_text, reply_to_message_id=query.message.message_id
            ...
            if img_url:
                ...
                await edit_rich_message_safe(
                    context.bot, chat_id=query.message.chat_id, message_id=query.message.message_id, html_content=caption, reply_markup=orig_kb
                )
            return
