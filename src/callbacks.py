...
async def start_command(update: Update, context):
    ...
    if args and args[0].startswith("ans_"):
        ...
        try:
            ...
            if existing_response:
                ...
                old_private_followup_mid = existing_response.get('private_followup_message_id')
                ...
                if old_private_mid:
                    try:
                        ...
                        pass
                if old_private_followup_mid:
                    try:
                        await context.bot.delete_message(chat_id=update.message.chat_id, message_id=old_private_followup_mid)
                    except Exception:
                        pass
                    ...
                    if latex_code:
                        ...
                        async with httpx.AsyncClient() as client:
                            ...
                            if resp and resp.status_code == 200:
                                ...
                                await asyncio.to_thread(db_update_private_message_ids, user_id, mid_key, private_message_id=m.message_id)
                                ...
                                if show_derivation or show_perf:
                                    ...
                                    follow_up = await send_rich_message_safe(
                                        context.bot, chat_id=update.message.chat_id, html_content=full_text, reply_to_message_id=m.message_id
                                    )
                ...
                await asyncio.to_thread(db_update_private_message_ids, user_id, mid_key, private_message_id=f_m.message_id)
                return
            ...
            traceback.print_exc()
            await send_rich_message_safe(context.bot, chat_id=update.message.chat_id, html_content="⚠️ Failed to load explanation.", reply_markup=channel_kb)
            return
    ...
    await send_rich_message_safe(
        ...
        html_content=(
            ...
            "To customize your study experience, select your academic grade level below:"
        ),
